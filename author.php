    ...
    <main id="content" role="main">
        ...
        <?php if ( have_posts() ) { ?>
            ...
            <header class="page-header author vcard h-card group" itemprop="author" itemscope itemtype="http://schema.org/Person">
                <h1 class="page-title"><?php printf( __( 'Articles by %s', fox_agency_text_domain() ), '<a class="url u-url fn p-fn n p-name" href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me author" itemprop="url"><span itemprop="name">' . get_the_author() . '</span></a>' ); ?></h1></header>
