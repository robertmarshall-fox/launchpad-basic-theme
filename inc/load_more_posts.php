...
function fox_agency_load_more_button( $button_text = '' ){
    ...
    if ( $wp_query->max_num_pages > 1 && get_next_posts_link() ){
        ...
        **/
        ...
        $load_type = 'button';
        if( get_theme_mod('fox_agency_infinite_scroll') ){
            $load_type = 'infinite';
        }
        
        
        ?>
        <div id="load-more-posts-error" class="load-more-posts-error error smooth"><p><?php echo esc_html__( 'Something has gone wrong. Please try again.', fox_agency_text_domain() );?></p></div>
        <button id="load-more-posts" class="load-more-posts-button" data-paged="<?php echo esc_attr__( $paged, fox_agency_text_domain() );?>" data-query='<?php echo json_encode ( $wp_query->query ) ;?>' data-custom-template="<?php echo $template;?>" data-loadtype="<?php echo $load_type;?>" style="display: none; opacity:0;"><?php echo esc_html__( $button_text, fox_agency_text_domain() );?></button>
    <?php
    }
}
...
function fox_agency_load_more_posts() {
...
add_action('wp_ajax_fox_agency_load_more_posts', 'fox_agency_load_more_posts');
add_action('wp_ajax_nopriv_fox_agency_load_more_posts', 'fox_agency_load_more_posts');